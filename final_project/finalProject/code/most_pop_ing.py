# -*- coding: utf-8 -*-
"""
Created on Tue May  5 15:19:00 2015
...
def main():
    ...
    for fp in file_paths:
        ...
        popular_dict = Counter()
        for ingredient in all_ingredients:
            popular_dict[ingredient] += 1
        #print popular_dict
        
        most_popular_lst = popular_dict.most_common(2)
        
        if most_popular_lst[0][1] == most_popular_lst[1][1]:
            most_popular = most_popular_lst[0][0].replace('_', ' ') + ' & ' +  most_popular_lst[1][0].replace('_', ' ')
        else:
            most_popular = most_popular_lst[0][0].replace('_', ' ')
        print most_popular
            
        most_popular_df = pd.DataFrame({'country': [country_name],
                                     'ingredient': [most_popular]})                            

        outfile_name = '/Users/laylamartin/Desktop/MSAN/Module4/Dataviz/ldmartin.github.io/final_project/plots/map_country/data/scirep-cuisines-detail/country_popular/' + country_name + '_popular.csv'       
        print "Writing popular ingredient file for", country_name, "to location", outfile_name
        
        most_popular_df.to_csv(outfile_name, index=False)
