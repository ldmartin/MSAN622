...
def get_most_popular(df, num_recipes, n=5):
    ...
    percent_recipes = np.round(most_pop_df.weight/float(num_recipes), decimals=2) 
...
#        outfile_name = '/Users/laylamartin/Desktop/MSAN/Module4/Dataviz/ldmartin.github.io/final_project/plots/map_country/data/scirep-cuisines-detail/country_popular_pairs/' + country_name + '_popular_pairs.csv'       
#        print "Writing network for", country_name, "to location", outfile_name
#        
#        most_popular.to_csv(outfile_name, index=False)
        
    # write number of recipes per country to file:
    n_recipe_dict = {'num_recipes': recipes_n,
                     'country': country_n}
    n_recipe_df = pd.DataFrame(n_recipe_dict)
#    n_recipe_df.to_csv('/Users/laylamartin/Desktop/MSAN/Module4/Dataviz/ldmartin.github.io/final_project/plots/map_country/data/scirep-cuisines-detail/num_recipes_by_country.csv', index=False)
